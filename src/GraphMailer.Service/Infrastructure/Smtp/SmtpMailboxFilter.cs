...
internal sealed class SmtpMailboxFilter : MailboxFilter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (size > 0 && size > maxSizeBytes)
        {
            ...
            await RecordRejectionSafeAsync(RejectionReasons.SizeExceeded, remoteIp, listenerPort);
        }
        ...
        if (!IpFilterService.IsAllowed(remoteIp, opts.IpWhitelist, opts.IpBlacklist))
        {
            ...
            await RecordRejectionSafeAsync(
                IpFilterService.IsBlacklisted(remoteIp, opts.IpBlacklist)
                    ? RejectionReasons.IpBlacklist
                    : RejectionReasons.IpNotWhitelisted,
                remoteIp, listenerPort);
            return false;
        }
            ...
            !context.Authentication.IsAuthenticated)
        {
            _logger.LogWarning(
                ...
                remoteIp);
            await RecordRejectionSafeAsync(RejectionReasons.AuthRequired, remoteIp, listenerPort);
            return false;
        }
        ...
        if (context.Properties.TryGetValue("Auth:Failed", out var failedUser))
        {
            _logger.LogWarning(
                ...
                remoteIp, failedUser);
            await RecordRejectionSafeAsync(RejectionReasons.PriorAuthFailed, remoteIp, listenerPort);
            return false;
        }
            ...
            if (username is not null)
            {
                ...
                if (restrictions is not null && !MailAddressFilter.IsAllowed(address, restrictions, []))
                {
                    ...
                    _logger.LogWarning(
                        ...
                        username, address);
                    await RecordRejectionSafeAsync(RejectionReasons.FromRestriction, remoteIp, listenerPort);
                    return false;
                }
            }
        }
        ...
        if (!MailAddressFilter.IsAllowed(address, opts.AllowedSenders, opts.BlockedSenders))
        {
            ...
            await RecordRejectionSafeAsync(RejectionReasons.BlockedSender, remoteIp, listenerPort);
            return false;
        }
        ...
        if (validation.Enabled && address != "@")
        {
            ...
            if (result == SenderLookupResult.Unknown)
            {
                ...
                _logger.LogWarning(
                    ...
                    address, remoteIp);
                await RecordRejectionSafeAsync(RejectionReasons.UnknownSender, remoteIp, listenerPort);
                return false;
            }
            ...
            if (result == SenderLookupResult.Indeterminate && validation.FailClosed)
            {
                _logger.LogWarning(
                    ...
                    address, remoteIp);
                await RecordRejectionSafeAsync(RejectionReasons.SenderValidationUnavailable, remoteIp, listenerPort);
                return false;
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!allowed)
        {
            ...
            await RecordRejectionSafeAsync(
                RejectionReasons.BlockedRecipient, remoteIp, SmtpMessageStore.GetListenerPort(context));
        }
        else
        {
    ...
    private async Task RecordRejectionSafeAsync(string reason, string clientIp, int listenerPort)
    {
        ...
        {
            _logger.LogWarning(ex, "[SmtpFilter] Failed to record rejection metric {Reason}", reason);
        }
    }
}
