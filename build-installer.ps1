...
.SYNOPSIS
    ...
    chains the .NET 10 Desktop Runtime prerequisite.

.DESCRIPTION
    1. Publishes the framework-dependent release via build-release.ps1 (flat folder).
    2. Downloads the .NET 10 Desktop Runtime (x64) installer and resolves its immutable URL
       (so the bootstrapper can fetch it on demand and stays small).
    3. Builds GraphMailer-<ver>.msi (service registration + ConfigTool shortcut). The MSI
       refuses to install when the Desktop Runtime 10 is missing, so the bare-MSI path
       cannot leave a non-startable installation behind.
...
# Netfx extension: version-tolerant .NET runtime detection (netfx:DotNetCoreSearch). Used by
# BOTH the MSI (launch condition) and the bundle (skip the prereq when already installed).
...
# ── 3. Download the .NET 10 Desktop Runtime + resolve its immutable URL ────────
Write-Host "Resolving .NET 10 Desktop Runtime (x64) ..." -ForegroundColor Green
$aka = 'https://aka.ms/dotnet/10.0/windowsdesktop-runtime-win-x64.exe'
...
# $netfxExt is resolved above — the MSI needs it too, for its runtime launch condition.
