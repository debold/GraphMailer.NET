...
[IP Filtering](../configuration/ip-filtering.md), and it makes **outbound** calls to Microsoft 365.
...
**Application Access Policy** — see [Entra / Graph Setup](../getting-started/entra-setup.md). You
...
warning, the remaining headers at Debug level. See [Logs](../monitoring/logs.md).
...
[Mail Queue](../configuration/mail-queue.md).

## Are my settings safe to back up and move to another machine?

Yes — use [Backup & Restore](../configuration/backup-restore.md). Backups are encrypted with their
...
On the [Graph API](../configuration/graph-api.md) page, use **Test Email Delivery**: enter a From
(a real tenant mailbox) and a To, and send. It uses the current settings, including unsaved changes.

## Related

- [Quickstart](../getting-started/quickstart.md) · [Troubleshooting](troubleshooting.md) · [Glossary](glossary.md)
