...
Common symptoms and what to do about them. The [Logs](../monitoring/logs.html) page is almost
...
- **Microsoft 365 not connected.** Until the [Entra / Graph Setup](../getting-started/entra-setup.html)
...
- **Outage within the retry window.** A transient Microsoft 365 / network problem is normal — the
  ...
  after that. See [Mail Queue](../configuration/mail-queue.html).
...
- **From address the tenant doesn't own.** The From must resolve to a real mailbox/alias in your
  tenant — see the [Quickstart](../getting-started/quickstart.html).

## A client cannot connect at all

- **Not whitelisted.** With a non-empty whitelist, only listed IPs/CIDRs may connect. Add the
  client's IP on [IP Filtering](../configuration/ip-filtering.html).
...
- **Wrong port / TLS mode.** Confirm the client uses a configured listener port and the matching
  encryption (plain 25, STARTTLS 587, implicit TLS 465). See [Servers & TLS](../configuration/servers-tls.html).
- **Service stopped.** Check the service state on the [Status](../monitoring/status.html) page.
...
> [Logs](../monitoring/logs.html)** (unknown user / wrong password / disabled). Look there.
...
> [Servers & TLS](../configuration/servers-tls.html) (select one, or create a self-signed one) and
> restart the service. Watch the [Logs](../monitoring/logs.html) for the certificate error.
...
different machine. Re-enter the affected secret (on [Graph API](../configuration/graph-api.html) or
[Access Control](../configuration/access-control.html)) and save. See the banner on
...
required”* badge — restart from the [Status](../monitoring/status.html) page. Everything else applies
immediately.
...
- [Logs](../monitoring/logs.html) · [Messages](../monitoring/messages.html) · [Status](../monitoring/status.html)
- [FAQ](faq.html) · [Glossary](glossary.html)
